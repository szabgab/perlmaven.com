...
Another advantage of Test::Simple: we can add names to the tests.

The Test::Simple module makes our life a bit more simple in that we don't have to write
our testing expression. In addition this new `ok` function can actually do some more.
It can get two arguments. The first one is `boolean` and it indicates success or failure of the test
